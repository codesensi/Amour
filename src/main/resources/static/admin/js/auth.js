/**
 * 后台管理登录态管理（前端 mock 实现）。
 *
 * 登录态由浏览器 localStorage 承载，结构与后端登录接口约定的返回体对齐：
 *   data: { accessToken: '访问令牌', expiresIn: 有效秒数，-1 代表永不过期 }
 * mock 阶段由 login.html 构造该结构写入本地；后端登录接口实现后，仅需将
 * login.html 中的 MOCK_LOGIN 开关改为 false，真实返回体可直接传入本文件，
 * 读写逻辑无需改动。
 */
(function (global) {
  'use strict';
   ...
   * 保存登录态。
   *
   * @param loginData 后端登录接口返回的 data：
   *                  { accessToken: 'xxx', expiresIn: 秒数，-1 代表永不过期 }
   * @param username  登录用户名（仅用于顶栏展示，可选）
   */
  function saveAuth(loginData, username) {
    var now = Date.now();
    var expiresIn = loginData && typeof loginData.expiresIn === 'number'
      ? loginData.expiresIn
      : NEVER_EXPIRES;
   ...
   */
  function setupAjaxGuard() {
    if (global.__ADMIN_AJAX_GUARD__) {
      return;
    }
    var $ = global.jQuery || (global.layui && global.layui.jquery);
    if (!$) {
      return;
    }
    global.__ADMIN_AJAX_GUARD__ = true;

    // 同源请求统一携带 accessToken（跨域请求不携带，避免令牌外泄）
    $.ajaxSetup({
      beforeSend: function (xhr, settings) {
        var crossOrigin = /^[a-z][a-z0-9+.-]*:\/\//i.test(settings.url)
