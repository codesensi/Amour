/**
 * 项目配置通用读取模块 —— 前端统一的项目配置（sys_config 表）查询口子（ES Module）。
 *
 * 适用场景：管理后台（站点名、头像、验证码开关）、门户（logo/页脚/版权等）等多处
...
function remoteLoad(keys) {
  const query = keys.map(function (key) { return encodeURIComponent(key); }).join(',');
...
export function getConfig(key) {
  return loadConfig([key]).then(function (map) { return map[key]; });
}
...
export function isOn(key) {
  return getConfig(key).then(function (value) { return String(value).toLowerCase() === 'true'; });
}

/**
 * 拼接 QQ 头像地址：nk 为 QQ 号，s 为尺寸（门户首页 640、留言列表 100），size 缺省 640。
 */
export function qqAvatar(qq, size) {
