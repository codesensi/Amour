...
CREATE TABLE IF NOT EXISTS `sys_config` (
    ...
    `value`       VARCHAR(4000) NULL DEFAULT NULL       COMMENT '配置值',
    `type`        VARCHAR(16)   NULL DEFAULT NULL   COMMENT '值类型: STRING, INTEGER, LONG, BOOLEAN',
    `group`       VARCHAR(64)   NULL DEFAULT NULL       COMMENT '分组',
    ...
    `del_flag`    TINYINT(1)    NULL DEFAULT 0          COMMENT '逻辑删除标识: 0-未删除, 1-已删除',
    PRIMARY KEY (`id`),
    UNIQUE INDEX `idx_sc_key` (`key` ASC)
    ) ENGINE = InnoDB
    CHARACTER SET = utf8mb4
    COLLATE = utf8mb4_general_ci
    COMMENT = '系统配置表'
